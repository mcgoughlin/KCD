...
class EnsembleClassifier(nn.Module):
    ...
    def forward(self,x,boundary=0.75,grad=False):
...
if __name__ == "__main__":
    home = "C:\\Users\\mcgoug01\\OneDrive - CRUK Cambridge Institute\\Python Scripts\\kits19\\classification\\NEW 10-fold_results\\static_fromscratch_3DwSpatialAugMaligClass"
    ...
    num_samples_perclass = 150
    
    data = []
    for psuedo_boundary in np.linspace(0.34,0.84,26):
        sens = 0
        spec = 0
        print("\nPseudoboundary: {:.2f}".format(psuedo_boundary))
    
        rand_n = meta[meta.label==1].sample(num_samples_perclass).filename.values
