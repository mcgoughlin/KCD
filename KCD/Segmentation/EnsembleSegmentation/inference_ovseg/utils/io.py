...
def _has_z_first(spacing, dims, filename):
    ...
    if spacing[0] == spacing[1]:
        ...
        if spacing[1] == spacing[2]:
            ...
            rounded_spacing = np.around(spacing,decimals=3)
            if rounded_spacing[0] == rounded_spacing[1]:
                if rounded_spacing[0] != rounded_spacing[2]:
                    has_z_first = False
                elif dims[0] == dims[1] and dims[0] != dims[2]:
                    # in the case of isotropic voxel we check the dimensions of the image instead
                    has_z_first = False
            ...
            if rounded_spacing[1] == rounded_spacing[2]:
                has_z_first = True
