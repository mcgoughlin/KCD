...
class SegmentationPreprocessing(object):
    ...
    def preprocess_raw_data(self,
        ...
        if isinstance(raw_data, str):
            ...
            raise ValueError('##SEG PREPROCESS##\nraw_data must be str if only infered from a sinlge folder or '
                             'list/tuple.')
        raw_ds_list = []
        ...
        if not self.is_initalised:
            print('##SEG PREPROCESS##\nPreprocessing classes were not initialised when calling '
        ...
        for raw_ds in raw_ds_list:
            ...
            for i in tqdm(range(len(raw_ds))):
                ...
                if exists(join(outfolder, 'images', scan)+".npy"):
                    if exists(join(outfolder, 'fingerprints', scan)+".npy"):
