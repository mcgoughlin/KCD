        ...
        if self.store_coords_in_ram:
            ...
            for ind in tqdm(range(len(self.vol_ds))):
                if self.store_data_in_ram:
                    ...
                    ims = np.load(self.vol_ds.path_dicts[ind][self.image_key])
                
                # ensure 4d array
                ims = maybe_add_channel_dim(ims)
                coords = self._get_bias_coords(ims)
            ...
            for ind, d in enumerate(self.vol_ds.path_dicts):
                case = os.path.basename(d[self.image_key])
                if case not in os.listdir(self.bias_coords_fol):
                    ims = np.load(d[self.image_key])
                    coords = self._get_bias_coords(ims)
    ...
    def __getitem__(self, index):
        ...
        if biased_sampling and cl >= 0:
            ...
            if self.store_coords_in_ram:
                ...
                # or hard drive
                case = os.path.basename(self.vol_ds.path_dicts[ind][self.image_key])
