...
import numpy as np
path = '/Users/mcgoug01/Library/CloudStorage/OneDrive-CRUKCambridgeInstitute/SecondYear/Classification/evaluate/benchmarking/csv_casewise'
save_dir = '/Users/mcgoug01/Library/CloudStorage/OneDrive-CRUKCambridgeInstitute/SecondYear/Classification/evaluate/high_level_results/benchmarking/csv_casewise'
dfs = []

size_dfs = []
for results in [file for file in os.listdir(path) if file.endswith('.csv') and file.startswith('kw')]:
    try:
        tempdf = pd.read_csv(os.path.join(path,results))
    except:
        print(results)
        continue
    _,model,size,epochs,fold,reading = results.split('_')
    epochs_kw,epochs_tw = np.array(epochs.split('epochs')[-1].split('-')).astype(int)
    fold = int(fold[-1])
    reading = int(reading.split('.')[0][-1])

    tempdf['Epochs_kw'] = [epochs_kw]*len(tempdf)
    tempdf['Epochs_tw'] = [epochs_tw]*len(tempdf)
...
for model_name in size_df.model.unique():
    ...
    for column in [col for col in final_df.columns if not (col in ['Epochs_kw','Epochs_tw','model','size'])]:
        col_matrix = final_df.groupby(['Epochs_kw','Epochs_tw','size']).mean()[column]
        count_df = final_df.groupby(['Epochs_kw','Epochs_tw','size']).count().AUC.rename('count')
        col_matrix =pd.concat([col_matrix, count_df], axis=1)
        if (column == 'AUC') or ('Sens' in column):
            col_matrix['std']= final_df.groupby(['Epochs_kw','Epochs_tw','size']).std()[column]
