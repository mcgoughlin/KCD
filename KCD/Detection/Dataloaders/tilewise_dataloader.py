...
class SW_Data(Dataset):
    def __init__(self, path, voxel_spacing_mm, thresh_r_mm,data_name=None, 
        ...
        home = join(voxel_path,"Threshold-"+str(thresh_r_mm)+"mm")
        print(home)
        assert(exists(home))
        ...
        self.data_df.case = self.data_df.case.str.replace('-','_')
        ...
        print("Training data contains {} benign tiles and {} malign tiles.".format(benign,malign))
    ...
    def _rotate(self,tensor,p=0.5):
        ...
        return torch.rot90(tensor,rot_extent)
    ...
    def __getitem__(self,idx:int):
        ...
        if type(self.test_case) == type(None):
            if self.is_train:
                transforms = [self._blur,self._add_noise,self._rotate,self._flip,self._contrast,self._flip]
...
def get_dataloader(home,voxel_spacing=2,data_name='test',
    ...
    dataset = SW_Data(home,voxel_spacing,data_name=data_name,is_3D=is_3D)        
    return DataLoader(dataset,batch_size=batch_size,shuffle=True)

def get_dataset(home,voxel_spacing=2,thresh_r_mm = 5,data_name='test',
                batch_size = 16,is_masked=False):
    return SW_Data(home,voxel_spacing,thresh_r_mm,data_name=data_name,is_masked=is_masked)        

if __name__ == '__main__':
    home = "/Users/mcgoug01/Library/CloudStorage/OneDrive-CRUKCambridgeInstitute/SecondYear/Classification/CNN_dataset"
    is_3D= False
    dataset = get_dataset(home,voxel_spacing=1,thresh_r_mm=10,data_name='coreg_ncct',is_masked=True)
    dataset.apply_foldsplit()
    dataset.is_train=False
    import matplotlib.pyplot as plt
    a,b = dataset[np.random.randint(0,1000)]
    print(b)
