    ...
    for reading in range(1):
        for split in range(1):
            ...
            for fold,train_index, test_index in fold_split: 
                ...
                paths = [MLP_path,svCNN_path,twCNN_path,GNN_path]
                ...
                # init models
                twCNN = model_generator.return_resnext(size=tw_size,dev=dev,in_channels=1,out_channels=3)
                ...
                torch.save(twCNN,os.path.join(twCNN_path,'model',twname))
                s1_tile_res,results = eval_.eval_twcnn(twCNN,test_tw_dl,dev=dev)
                cv_results.append(results)
                s1_tile_res.to_csv(os.path.join(twCNN_path,'csv',twname+'_{}.csv'.format(reading)))
    
CV_results = pd.concat(cv_results, axis=0, ignore_index=True)
print(CV_results.columns)
ROC = eval_.ROC_func(CV_results['Top-10'],CV_results['label'],max_pred=10,intervals=1000)
np.save(os.path.join(split_path,'Tile Model_ROC'),ROC)
