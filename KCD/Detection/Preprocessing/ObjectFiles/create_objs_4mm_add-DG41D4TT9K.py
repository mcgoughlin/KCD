...
def plot_single_kidney(im,index,kidney_cent,kidney_label,
                       bone_cent,cent_4mm,cancer_cent,
                       cyst_cent,is_kits19=True):
    ...
    plt.scatter(cancer_cent[:,lr_index],cancer_cent[:,0], c = 'cyan',s=5, label='cancer')
    if len(cyst_cent)>0:
        plt.scatter(cyst_cent[:,lr_index],cyst_cent[:,0], c = 'green',s=5, label='cyst')
    plt.axis('off')
    
def plot_double_kidney(im,index,right_kidney,left_kidney,
                       left_4mm,right_4mm,
                       cancer_cent,cyst_cent,is_kits19=True):
...
def plot_all_single_kidney(im, centre, kidney_centre, 
                           ...
                           cent_4mm,cancer_cent, cyst_cent,is_kits19=True):
    ...
    plot_single_kidney(im,int(centre[axial_index])-10,kidney_centre,'central_kidney',
                       bone_cent,cent_4mm,cancer_cent,cyst_cent,is_kits19=is_kits19)
...
def plot_all_double_kidney(im,centre,right_kidney,
    ...
    is_kits19 = (dataset=='kits_ncct') or (dataset=='all_add')
    if is_kits19:axial_index = 2
    else:axial_index=1
    ...
    plot_double_kidney(im,int(centre[axial_index]-10),right_kidney,
                       left_kidney,left_4mm,right_4mm,
                       lb_cancers,lb_cysts,is_kits19=is_kits19)
...
def seg_2_mesh(segmentation,dataset='kits_ncct', show=True):
    if (dataset=='kits_ncct') or (dataset=='coreg_ncct') or (dataset=='all_add'):
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        if (dataset=='coreg_ncct'):
            lb_p = '/Users/mcgoug01/Library/CloudStorage/OneDrive-CRUKCambridgeInstitute/SecondYear/Segmentation/seg_data/raw_data/all_add/labels/'
        ...
        ########### LOAD DATA #############
            ...
            if case.startswith('KiTS'):
                case_fp = os.path.join(lb_p,'case_{:05d}.nii.gz'.format(case_num))
                lb_n = nib.load(case_fp)
            else:
            ...
            if is_kits19ncct or (dataset=='coreg_ncct') or (dataset=='all_add'):
                z_spac = inf_n.header['pixdim'][3]
                inplane_spac = np.mean(np.abs(inf_n.header['pixdim'][1:3]))
                label_spacing = np.prod(lb_n.header['pixdim'][1:4])
            else:
        ...
        ########### GENERATE FEATURES #############
            ...
            lr_index = assign_lrindex(is_kits19ncct or (dataset=='coreg_ncct')or (dataset=='all_add'))
            ...
            if len(inference_centroids)==1:
                ...
                central_kidney_flag, ud_bone, lr_bone = is_sole_kidney_central(inference_centroids,im,inf,
                                                                               inf_n.header['pixdim'][3], is_kits19=is_kits19ncct or (dataset=='coreg_ncct'))
            ...
            if not (dataset=='coreg_ncct'):
                if not ((lb.shape[0]==512) and (lb.shape[1] == 512)): 
                    print("Strange im shape:",lb.shape)
                    continue
        
            if is_kits19ncct:
                # swap label dimensions between the z and y axis for the cancer and cyst centroids to realign with kidney inferences
                lb_cancers = np.array([np.array([centroid[0],centroid[2],centroid[1]]) for _, centroid in get_masses(lb==2,20)])
                try:
                    lb_cysts = np.array([np.array([centroid[0],centroid[2],centroid[1]]) for _, centroid in get_masses(lb==3,20)])
                except IndexError:
                    lb_cysts = []
                    print("No cysts found.")
            else:
                lb_cancers = np.array([[*centroid] for _, centroid in get_masses(lb==2,20)])
                try:
                    lb_cysts = np.array([[*centroid] for _, centroid in get_masses(lb==3,20)])
                except IndexError:
                    lb_cysts = []
                    print("No cysts found.")
            ...
            if single_kidney_flag:
                if central_kidney_flag:
                    canc2kid = assign_labels_2_kidneys(np.array([central_kidney]),lb_cancers)
                    cyst2kid = assign_labels_2_kidneys(np.array([central_kidney]),lb_cysts)
                else:
                    if type(left_kidney) == type(None):
                        canc2kid = assign_labels_2_kidneys(np.array([right_kidney]),lb_cancers)
                        cyst2kid = assign_labels_2_kidneys(np.array([right_kidney]),lb_cysts)
                    else:
                        canc2kid = assign_labels_2_kidneys(np.array([left_kidney]),lb_cancers)
                        cyst2kid = assign_labels_2_kidneys(np.array([left_kidney]),lb_cysts)
            else:
                canc2kid = assign_labels_2_kidneys(np.array([left_kidney,right_kidney]),lb_cancers)
                cyst2kid = assign_labels_2_kidneys(np.array([left_kidney,right_kidney]),lb_cysts)
            
            cancer_vols = np.asarray([im.area*label_spacing for im,centroid in get_masses(lb==2,20)])
            cyst_vols = np.asarray([im.area*label_spacing for im,centroid in get_masses(lb==3,20)])
            ...
            if single_kidney_flag:
                if central_kidney_flag:
                    for i,inf in enumerate([cent_stats]):
                        ...
                        for i in range(len(cancer_vols)):
                            entry['cancer_{}_vol'.format(i)] = cancer_vols[i]
                            
                        for i in range(len(cyst_vols)):
                            entry['cyst_{}_vol'.format(i)] = cyst_vols[i]
                ...
                for i,(intens,inf) in enumerate(zip([left_intensity,right_intensity],[left_stats,right_stats])):
                    ...
                    for j in range(len(canc2kid)):
                        if canc2kid[j] != i: continue
                        entry['cancer_{}_vol'.format(canc_counter)] = cancer_vols[j]
                        canc_counter+=1
                            
                    cyst_counter = 0
                    for j in range(len(cyst2kid)):
                        if cyst2kid[j] != i: continue
                        entry['cyst_{}_vol'.format(cyst_counter)] = cyst_vols[j]
                        cyst_counter+=1
        ...
        ########### TESTING #############
            ...
            if is_testing: 
                ...
                for vol,assoc in zip(cancer_vols,canc2kid):
                    if central_kidney_flag:string = 'centre'
                    elif assoc ==1: string = 'right'
                    else: string = 'left'
                    print("Cancer has a volume of {:.3f}cm cubed, and belongs to the {} kidney.".format(vol/1000,string))
            
                for vol,assoc in zip(cyst_vols,cyst2kid):
                    if central_kidney_flag:string = 'centre'
                    elif assoc ==1: string = 'right'
                    else: string = 'left'
                    print("Cyst has a volume of {:.3f}cm cubed, and belongs to the {} kidney.".format(vol/1000,string))
        
                # Plotting images for testing
                if single_kidney_flag:
                    if central_kidney_flag:
                        plot_all_single_kidney(im,centre,central_kidney,'central_kidney',
                                           [ud_bone,lr_bone],cent_4mm,lb_cancers,lb_cysts,
                                           is_kits19=is_kits19ncct or (dataset=='coreg_ncct')or (dataset=='all_add'))
                    else:
                        if type(left_kidney) == type(None):
                            plot_all_single_kidney(im,centre,right_kidney,'left_kidney',
                                               [ud_bone,lr_bone],right_4mm,lb_cancers,lb_cysts,
                                               is_kits19=is_kits19ncct or (dataset=='coreg_ncct'))
                        else:
                            plot_all_single_kidney(im,centre,left_kidney,'right_kidney',
                                               [ud_bone,lr_bone],left_4mm,lb_cancers,lb_cysts,
                                               is_kits19=is_kits19ncct or (dataset=='coreg_ncct'))
