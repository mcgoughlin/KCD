...
if __name__ == "__main__":
    ...
    for is_kits19ncct in [False,True]:  
        ...
        for obj in os.listdir(obj_path):
            ...
            bpy.context.view_layer.objects.active = bpy.data.objects[obj[:-4]]
    
            obj_file = bpy.data.objects[obj[:-4]]
            num_vertices = len(obj_file.data.vertices)
            ...
            curvatures= mean_curvature( obj_file)
            
            vertices = np.asarray([[vert.co.x, vert.co.y, vert.co.z] for vert in bpy.context.object.data.vertices])
    
            edges = np.asarray([[vert for vert in polygon.vertices] for polygon in bpy.context.object.data.edges])
            
            for bin_, name in zip(*get_hist(np.abs(curvatures))):
                name = 'curv'+str(name)
                entry[name] = bin_
            
            np.save(os.path.join(save_path,'curvatures',obj[:-4]),curvatures)
            np.save(os.path.join(save_path,'vertices',obj[:-4]),vertices)
            np.save(os.path.join(save_path,'edges',obj[:-4]),edges)
            bpy.ops.export_scene.obj(filepath=os.path.join(save_path,"cleaned_objs",obj))
            os.remove(os.path.join(save_path,"cleaned_objs",obj[:-4]+'.mtl'))
