...
from array_manipulation_utils_kw_nolabel import *
import numpy as np
import os
...
def filename_structure(path,name,voxel_size_mm,thresh,depth):
    ...
    fold = os.path.join(fold,"3D-Depth"+str(depth)+'mm')
    if not os.path.exists(fold): os.mkdir(fold)
...
def nifti_2_correctarr(im_n):
    ...
    penult = sitk.GetArrayViewFromImage(flip_im)
    nda = np.rot90(penult,k=2,axes=(1,2))
...
def is_sole_kidney_central(kidney_centroids, im,inf, inplane_spac,
    ...
    lr_bone,ud_bone,z_bone = regionprops((im>250).astype(int))[0].centroid
...
def get_shifted_windows(reshaped_im,reshaped_seg,
                        overlap=None,patch_size=None,boundary_z=5):
    

    sw_reshape_im = extract_sliding_windows(reshaped_im, overlap=overlap, patch_size=patch_size,boundary_z=boundary_z)
    sw_reshape_seg = extract_sliding_windows(reshaped_seg, overlap=overlap, patch_size=patch_size,boundary_z=boundary_z)  
...
def get_centralised_windows(reshaped_im,reshaped_seg,centroid,patch_size=None,is_kits=True,
    ...
    if is_kits:
        z_list = np.arange(0,shape[1]-patch_size[1],boundary_z)
        ...
        central_window_im = np.array([reshaped_im[top_left_x:bottom_right_x,
                                          top_z+i:bottom_z+i,
                                          top_left_y:bottom_right_y] for i in z_list])
        ...
        bottom_right_x = min(shape[0],top_left_x+patch_size[0])
        bottom_z = patch_size[2]
        
        if bottom_right_x == shape[0]: top_left_x = bottom_right_x-patch_size[0]
        if bottom_right_y == shape[1]: top_left_y = bottom_right_y-patch_size[1]
        
        z_list = np.arange(0,shape[2]-patch_size[2],boundary_z)
        central_window_im = np.array([reshaped_im[top_left_x:bottom_right_x,                                
...
def filter_shifted_windows(sw_im,sw_seg,kid_thresh,target_spacing,shuffle=True):
    ...
    none = sw_im[labels==0]
    none = none[none.max(axis=-1).max(axis=-1).max(axis=-1)>-200]
...
def save_windows(windowed_im,windowed_seg,
                 ...
                 thresh_r_mm,case_name,kidney_side='random',patch_size=(20,224,224),
                 shuffle=True,save_limit=100,centralised=False,depth=10):
    
    if centralised: string = 'centralised'
    else: string = 'shifted'
    
    patch_size= tuple([depth,*patch_size[1:]])
    ...
    for subname, label_group in zip(names,sw_subtypes):
        fold = filename_structure(save_path,subname,voxel_size_mm,thresh_r_mm,depth)
        for i,sw in enumerate(label_group):
            if i >= save_limit: break
            sw = np.expand_dims(np.expand_dims(sw,axis=0),axis=0)
            # print(subname,sw.shape,sw.shape[-3:],patch_size)
            assert(sw.shape[-3:]==patch_size)
...
def create_dataset(int_list,im_path,seg_path,target_spacing,overlap,
                   ...
                   voxel_size_mm, save_limit,is_3D=True,
                   bbox_boundary_mm=50,depth=10,boundary_z=5):
    ...
    for get_int in int_list:
        ...
        # kid_thresh = ((20/target_spacing[0])**3) * 3.1416*(4/3)
        kid_thresh = ((10/target_spacing[0])**3) * 3.1416*(4/3)
        ...
        for kidney_datum, name in zip(kidney_data,kid_str):
            ...
            cent_im, cent_seg = get_centralised_windows(ct,seg,centroid,patch_size=patch_size,is_kits=is_kits,boundary_z=boundary_z) 
            if is_kits:
                cent_im = np.swapaxes(cent_im,1,2)
                cent_seg = np.swapaxes(cent_seg,1,2)
            else:
                cent_im = np.swapaxes(cent_im,1,3)
                cent_seg = np.swapaxes(cent_seg,1,3)
            assert(cent_im.shape[-3:]==(depth,224,224))
            assert(cent_seg.shape[-3:]==(depth,224,224))
            save_windows(cent_im,cent_seg,kid_thresh,
                         target_spacing,save_path,
                         voxel_size_mm,thresh_r_mm,get_int,patch_size=tuple(patch_dims),kidney_side=name,
                         shuffle=False,save_limit=save_limit,centralised=True,depth=depth)
                
                
def create_3D(im_path,seg_path, voxel_size_mm,
              overlap_mm, patch_voxels, save_dir,depth=10,
              thresh_r_mm = 7.5,save_limit=1000,
              train_fraction=0.8,data_name = False,bbox_boundary_mm=50,boundary_z=5):
    ...
    create_dataset(case_integers,data_path,seg_path,target_spacing,overlap,
                   ...
                   bbox_boundary_mm=bbox_boundary_mm,depth=depth,boundary_z=boundary_z)
...
if __name__ == "__main__":
    ...
    boundary_z = 5 # this dictates the z-wise gap between shifted /centralised windows in mm and voxel slices (if voxel=1mm)
    patch2d = 224
    depth = 20
    ...
    for thresh in thresholds_r_mm:
        for spacing in voxel_spacings: 
            ...
            create_3D(path,segpath,spacing,overlap_mm,patch2d,save_dir,data_name='add_ncct_unseen',depth=depth,
                      thresh_r_mm=thresh, save_limit=save_limit_percase_perlabel,bbox_boundary_mm=40,boundary_z=boundary_z)