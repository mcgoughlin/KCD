...
def get(case_string,cases_folder):
    case_path = os.path.join(cases_folder, "images", case_string)
    seg_path = os.path.join(cases_folder, "labels", case_string)
...
def extract_labels(sw_seg, tumour_vol_thresh = None, kidney_vol_thresh = None):
    
    labels = sw_seg.reshape(sw_seg.shape[0],-1)
    counts = np.apply_along_axis(count_elements,-1,labels)
    # return counts
    return np.array([2 if ((tums>tumour_vol_thresh) and (kids>kidney_vol_thresh)) else 1 if kids>kidney_vol_thresh else 0 for kids,tums in counts])
